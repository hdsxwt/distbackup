...
设为 Source 时还会分配一个唯一身份识别码（UUID），用于血统强制检查。

详见 [feature_zh.md](./feature_zh.md) 了解仓库身份识别、安全保护等特性的完整文档。
...
目录/
  .backup/                  # Windows 下自动隐藏
    config.json             # {"type": "Target"}  或  {"type": "Source", "repo_id": "uuid"}
...
4. 快照下拉框显示可用快照，`(new)` 标记表示最新快照。下拉选择即可加载。
5. **Compare** 查看差异树形列表。
6. **Start Backup** 执行同步。安全检查包括类型强制、快照 root 校验、
   血统强制检查、快照过期警告等。
...
| `SnapshotManager` | 保存/加载时间戳命名的 JSON 快照。管理 `config.json` 仓库类型、身份识别码与 `root` 校验。隐藏 `.backup/` |
| `Differ` | 比较两个 `{路径: 哈希}` 映射 -> 新增/修改/仅目标/未变化 |
| `Syncer` | 复制新增和修改的文件到目标。拦截路径穿越。支持 `dry-run`。不删除 |
| `_win32` | Windows 专用辅助（隐藏目录，记录日志错误） |

## 运行环境

- Python 3.10+
- 仅标准库 -- `hashlib`、`os`、`shutil`、`json`、`tkinter`、`argparse`、`uuid`
- 145 个测试，全部通过